...
Modeler.extend = function (type, definition, obj, json) {
  ...
  for (var p in definition) {
    ...
    (function () {
      var propertyName = p;
      var validator = prop.validation || function () {
        return true;
      };
      var validateFunction = prop.validation || function () {
        return true;
      };
      var filter = prop.filter || function (v) {
        return v;
      };
      var filterFunction = prop.filter || function (v) {
        return v;
      };
      ...
      if (typeof validator != 'function') {
        validateFunction = function (v) {
          ...
          if ((validator.looksLike) && (typeof v == 'string')) {
            ...
            if ((validator.looksLike === 'postcode') && !((/[A-Za-z]{1,2}\d{1,2}\s*\d[A-Za-z][A-Za-z]/).test(v))) return false;
            if ((validator.looksLike === 'money') && !((/(\d)*\.\d\d/).test(v))) return false;
          }
  ...
  for (var i = 0; i < jsonProperties.length; i++) {
    ...
    if (!definition.hasOwnProperty(someJsonProperty)) {
      if (Modeler.Settings.debug) {
        console.log('Unexpected', type + '.' + someJsonProperty, '->', (typeof json[someJsonProperty]) + ((json[someJsonProperty] && json[someJsonProperty].hasOwnProperty('__type')) ? ('/' + json[someJsonProperty].__type) : ''), JSON.stringify(json[someJsonProperty]));
      }
    }
  }
